    ...
    <div className="space-y-10 animate-in-up">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-white/50 text-lg mt-1 font-medium font-body">Browse and manage the complete LibraFlow collection.</p>
        </div>
        <div className="flex gap-3">
          <Button onClick={() => setIsExportModalOpen(true)} variant="outline" className="bg-white/5 border-white/10 rounded-2xl h-12 px-6 font-headline font-semibold text-white hover:bg-white/10">
            <Download className="w-4 h-4 mr-2" /> EXPORT LEDGER
          </Button>
          <Button onClick={handleAddNew} className="gradient-primary rounded-2xl h-12 px-6 font-headline font-semibold shadow-lg hover:scale-[1.02] transition-transform">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center glass-card p-4 rounded-[24px]">
        <div className="relative flex-1 group w-full">
          ...
          <Input 
            ...
            className="pl-12 h-14 bg-white/5 border-white/5 rounded-xl focus:border-primary/50 text-lg font-body"
            value={searchTerm}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0 w-full lg:w-auto no-scrollbar">
          <Badge 
            ...
            className={cn(
              "h-14 px-6 rounded-xl cursor-pointer transition-all border border-white/5 shrink-0 uppercase tracking-widest font-bold text-[10px] font-headline",
              activeCategory === null 
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="glass-card rounded-[40px] p-20 text-center space-y-8 animate-in-up">
          ...
          <div className="space-y-2">
            ...
            <p className="text-white/30 text-lg max-w-sm mx-auto font-medium font-body">Your search criteria didn't yield any results in the current archive.</p>
          </div>
      ...
      <Dialog open={isExportModalOpen} onOpenChange={setIsExportModalOpen}>
        <DialogContent className="glass-card border-white/10 rounded-[32px] sm:max-w-[450px] p-0 overflow-hidden shadow-2xl">
          <DialogHeader className="p-8 bg-white/5 border-b border-white/5">
            ...
            <DialogDescription className="text-white/40 font-body">Select your preferred format for the inventory report.</DialogDescription>
          </DialogHeader>
          <div className="p-8 grid grid-cols-1 gap-4 font-body">
            <Button onClick={() => handleExport('PDF')} variant="ghost" className="h-20 justify-start gap-6 rounded-2xl bg-white/5 border border-white/5 hover:bg-primary transition-all group">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40 uppercase tracking-widest font-bold font-headline">.PDF Archive</p>
              </div>
            </Button>
            <Button onClick={() => handleExport('CSV')} variant="ghost" className="h-20 justify-start gap-6 rounded-2xl bg-white/5 border border-white/5 hover:bg-primary transition-all group">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40 uppercase tracking-widest font-bold font-headline">.CSV Ledger</p>
              </div>
            </Button>
            <Button onClick={() => handleExport('Excel')} variant="ghost" className="h-20 justify-start gap-6 rounded-2xl bg-white/5 border border-white/5 hover:bg-primary transition-all group">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40 uppercase tracking-widest font-bold font-headline">.XLSX Sheet</p>
              </div>
            </Button>
          </div>
        </DialogContent>
